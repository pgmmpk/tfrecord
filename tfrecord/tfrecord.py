...
# based on http://www.machinelearninguru.com/deep_learning/tensorflow/basics/tfrecord/tfrecord.html
...
def compression_code(compression):
    ...
    if code is None:
        raise ValueError('Unknown or unsupported compression type: ' + compression)

class Writer:
    def __init__(self, fname, pack_sample, compression=None):
        self._engine = tf.python_io.TFRecordWriter(fname, compression_code(compression))
...
class Reader:
    def __init__(self, fname, unpack_sample, compression=None):
        self._engine = iter(tf.python_io.tf_record_iterator(fname, compression_code(compression)))
